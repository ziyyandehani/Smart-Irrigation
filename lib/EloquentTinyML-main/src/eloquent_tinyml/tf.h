    ...
    namespace TF {
        ...
        public:
            ...
            Exception& begin(const unsigned char *data) {
                ...
                String msg = String("Model version mismatch. Expected ") + String(TFLITE_SCHEMA_VERSION) + ", got " + String(model->version());
                return exception.set(msg);
                }
...
//                ESP_LOGI("TF", "this->input.offset=%.4f, this->input.scale=%.4f", inputOffset, inputScale);
            ...
            Exception& predict(int8_t *x) {
                ...
                if (interpreter->Invoke() != kTfLiteOk)
                    return exception.set(String("Invoke() failed"));
